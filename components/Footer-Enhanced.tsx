...
export default function FooterEnhanced({ lang }: { lang: string }) {
  const content = footerContent[lang] || footerContent.en;
  const isRTL = lang === 'ar';

  return (
    <footer className={`bg-gray-900 text-gray-300 ${isRTL ? 'rtl' : 'ltr'}`}>
      {/* Trade Information Banner */}
      <div className="bg-blue-900 border-t-4 border-blue-600 px-4 sm:px-6 lg:px-8 py-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            {/* Export Markets */}
            <div className={isRTL ? 'text-right' : ''}>
              <h5 className="text-white font-bold mb-2">
                {lang === 'en' && '🌍 Export Markets'}
                {lang === 'es' && '🌍 Mercados de Exportación'}
                {lang === 'de' && '🌍 Exportmärkte'}
                {lang === 'fr' && '🌍 Marchés d\'exportation'}
                {lang === 'ar' && '🌍 أسواق التصدير'}
              </h5>
              <p className="text-blue-200">
                {lang === 'en' && 'Africa • Southeast Asia • Europe'}
                {lang === 'es' && 'África • Sudeste Asiático • Europa'}
                {lang === 'de' && 'Afrika • Südostasien • Europa'}
                {lang === 'fr' && 'Afrique • Asie du Sud-Est • Europe'}
                {lang === 'ar' && 'أفريقيا • جنوب شرق آسيا • أوروبا'}
              </p>
            </div>
            ...
            <div className={isRTL ? 'text-right' : ''}>
              <h5 className="text-white font-bold mb-2">
                {lang === 'en' && '📦 Trade Terms'}
                {lang === 'es' && '📦 Términos Comerciales'}
                {lang === 'de' && '📦 Handelsbedingungen'}
                {lang === 'fr' && '📦 Conditions Commerciales'}
                {lang === 'ar' && '📦 شروط التجارة'}
              </h5>
            ...
            <div className={isRTL ? 'text-right' : ''}>
              <h5 className="text-white font-bold mb-2">
                {lang === 'en' && '🚢 Shipping Ports'}
                {lang === 'es' && '🚢 Puertos de Envío'}
                {lang === 'de' && '🚢 Versandhäfen'}
                {lang === 'fr' && '🚢 Ports d\'expédition'}
                {lang === 'ar' && '🚢 موانئ الشحن'}
              </h5>
              <p className="text-blue-200 text-xs">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 ${isRTL ? 'flex flex-col-reverse' : ''}`}>
          ...
          <div className={isRTL ? 'text-right' : ''}>
            ...
            <p className="text-sm leading-relaxed mb-4">
              {lang === 'en' && 'Professional wholesale supplier of solar systems, home appliances, and 3C electronics for global buyers.'}
              {lang === 'es' && 'Proveedor mayorista profesional de sistemas solares, electrodomésticos y electrónica 3C para compradores globales.'}
              {lang === 'de' && 'Professioneller Großhandelslieferant von Solarsystemen, Haushaltsgeräten und 3C-Elektronik für globale Käufer.'}
              {lang === 'fr' && 'Fournisseur en gros professionnel de systèmes solaires, d\'électroménagers et d\'électronique 3C pour les acheteurs mondiaux.'}
              {lang === 'ar' && 'مورد بالجملة احترافي للأنظمة الشمسية والأجهزة المنزلية والإلكترونيات 3C للمشترين العالميين.'}
            </p>
            ...
            <ul className="space-y-2">
              ...
              <li>
                <Link href={`/${lang}/regions`} className="hover:text-white transition-colors text-sm font-semibold text-blue-400">
                  ...
                  {lang === 'fr' && '🌍 Marchés Régionaux'}
                  {lang === 'ar' && '🌍 الأسواق الإقليمية'}
                </Link>
              </li>
            </ul>
          </div>
            ...
            <ul className="space-y-3">
              {content.contact.map((item) => (
                <li key={item.label} className="text-sm">
                  ...
                  {item.href ? (
                    <a href={item.href} className="text-blue-400 hover:text-blue-300 ml-2 break-all">
                      {item.value}
                    </a>
                  ) : (
