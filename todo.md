...
- [ ] 新增一篇专业 B2B 文章并嵌入用户提供的 HousePlus 工厂便携式电源视频，完成视频 SEO、可访问性、Schema、Sitemap 与生产验收
- [ ] 使用已确认的 `houseplus-images` R2 桶上传视频、海报与字幕，并替换文章中的临时媒体地址后完成生产验收
- [x] 核查生产网页最新图片的实际 URL、代码引用、DNS/缓存链路及对应存储源，确认后再制定无损迁移方案
- [x] 备份旧 R2 文件、媒体数据库元数据、多语言 SEO 字段与旧 URL 映射
- [ ] 将全部可核验媒体无损迁移到第二个账户的 `houseplus-images`，建立统一媒体 Worker 与回滚映射
- [ ] 切换生产媒体路由并验证图片、图片 Sitemap、文章、产品页与视频资源后再评估旧链路删除
- [ ] 独立复核媒体存储、域名、Worker、缓存、备份权限、完整性与回滚条件，确认无误导和遗漏
- [x] 以生产网站当前实际显示的全部图片为基准，建立文件、引用位置、元数据、五语言 SEO 信息与旧 URL 的全量迁移清单
- [ ] 全量迁移完成后逐页逐资源校验，确保图片显示、旧 URL 映射、图片 Sitemap、文章/产品 Schema 与视频资源可回滚
